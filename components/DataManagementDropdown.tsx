...
export const DataManagementDropdown: React.FC<DataManagementDropdownProps> = ({ onBackup, onRestore }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <DatabaseIcon className="w-5 h-5 sm:w-6 sm:h-6 text-gray-700 dark:text-gray-300" />
        <span className="hidden sm:inline text-sm font-semibold text-gray-700 dark:text-gray-300">Dados</span>
        <ChevronDownIcon className={`hidden sm:block w-4 h-4 text-gray-500 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-20 animate-fade-in-down">
          <ul className="py-2">
            <li className="px-4 py-2 text-xs font-semibold text-gray-400 uppercase">Gerenciar Dados</li>
            <li>
              <button
                onClick={onBackup}
