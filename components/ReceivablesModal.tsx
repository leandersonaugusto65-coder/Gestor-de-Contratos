...
export const ReceivablesModal: React.FC<ReceivablesModalProps> = ({ clients, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-gray-900/75 dark:bg-black/70 z-50 flex justify-center items-center p-4">
            <div className="bg-slate-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-fade-in-up">
                <header className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">Notas Pendentes</h2>
                ...
                <div className="p-4 sm:p-6 flex-grow overflow-y-auto bg-slate-100 dark:bg-gray-900/30">
                    {unpaidInvoices.length > 0 ? (
                        <>
                            ...
                            <div className="hidden sm:block overflow-x-auto">
                                <table className="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                                    <thead className="text-xs text-gray-500 dark:text-gray-400 uppercase bg-slate-200 dark:bg-gray-700">
                                        <tr>
                                    ...
                                    <tbody>
                                        {unpaidInvoices.map(({clientName, biddingId, invoice, invoiceValue}) => (
                                            <tr key={invoice.id} className="bg-slate-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-slate-100 dark:hover:bg-gray-700">
                                                <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{clientName}</td>
                                                ...
                                                <td className="px-4 py-3 text-right font-semibold font-mono text-yellow-600 dark:text-yellow-500">{formatCurrency(invoiceValue)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <div className="sm:hidden space-y-3">
                                {unpaidInvoices.map(({clientName, biddingId, invoice, invoiceValue}) => (
                                    <div key={invoice.id} className="bg-slate-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 shadow-sm">
                                        <h3 className="text-gray-900 dark:text-white font-bold text-xs mb-1">{clientName}</h3>
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-[10px] text-gray-500 dark:text-gray-400 uppercase">Lic: {biddingId}</span>
                                            <span className="text-[10px] font-bold text-green-700 dark:text-green-400">NF {invoice.invoiceNumber}</span>
                                        </div>
                                        <div className="flex justify-between items-baseline pt-2 border-t border-gray-200 dark:border-gray-700">
                                            <span className="text-[10px] text-gray-400 dark:text-gray-500">{formatDate(invoice.date)}</span>
                                            <span className="text-sm font-bold text-yellow-600 dark:text-yellow-500 font-mono">{formatCurrency(invoiceValue)}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <p className="text-center text-gray-500 dark:text-gray-400 py-12">Não há notas pendentes de pagamento.</p>
                    )}
                </div>
