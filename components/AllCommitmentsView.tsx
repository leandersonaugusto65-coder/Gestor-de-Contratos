...
export const AllCommitmentsView: React.FC<AllCommitmentsViewProps> = ({ commitments, onSelectCommitment }) => {
    ...
    if (commitments.length === 0) {
        return <div className="text-center py-12 bg-slate-100 dark:bg-gray-800 rounded-lg shadow-inner"><p className="text-gray-500 dark:text-gray-400">Nenhum empenho pendente encontrado.</p></div>;
    }
                ...
                <table className="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                    ...
                    <tbody>
                        {commitments.map(commitment => (
                            <tr key={commitment.id} className="bg-slate-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-slate-100 dark:hover:bg-gray-700 cursor-pointer" onClick={() => onSelectCommitment(commitment)}>
                                <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{commitment.clientName}</td>
                                ...
                                <td className="px-4 py-3 text-right font-semibold font-mono text-yellow-600 dark:text-yellow-500">{formatCurrency(calculateCommitmentValue(commitment))}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                {commitments.map(commitment => (
                    ...
                    >
                        <div className="flex justify-between items-start mb-2">
                            <h3 className="text-gray-900 dark:text-white font-bold text-sm line-clamp-1 flex-1">{commitment.clientName}</h3>
                            <span className="text-[10px] bg-yellow-100 dark:bg-yellow-600/20 text-yellow-700 dark:text-yellow-500 px-2 py-0.5 rounded font-bold uppercase ml-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <div className="col-span-2 pt-2 border-t border-gray-200 dark:border-gray-700 mt-1 flex justify-between items-center">
                                <p className="text-[10px] uppercase font-semibold text-gray-400 dark:text-gray-500">Valor do Empenho</p>
                                <p className="text-sm font-bold text-yellow-600 dark:text-yellow-500 font-mono">{formatCurrency(calculateCommitmentValue(commitment))}</p>
                            </div>
                        </div>
                    </div>
                ))}
