...
export const ResetPasswordModal: React.FC<ResetPasswordModalProps> = ({ supabaseClient, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/75 dark:bg-black/70 z-50 flex justify-center items-center p-4">
      <div className="bg-slate-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl w-full max-w-md animate-fade-in-up">
        <form onSubmit={handlePasswordReset}>
          <div className="p-6">
            ...
            <div>
              ...
              <input 
                ...
                className="mt-1 block w-full px-3 py-2 bg-slate-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md text-sm shadow-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-yellow-600"
                autoFocus
              />
            </div>
          </div>
          <div className="bg-slate-100 dark:bg-gray-900 px-6 py-3 flex justify-end space-x-3 rounded-b-lg">
            ...
            <button type="submit" disabled={isLoading} className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-yellow-600 hover:bg-yellow-700 disabled:bg-yellow-400 disabled:cursor-not-allowed">
