        ...
        <div className="p-4 sm:p-6 flex-grow overflow-y-auto bg-gray-900/30">
          {isLoading ? (
            ...
            <div className="space-y-3">
              {users.map(user => (
                <div key={user.id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-white truncate" title={user.full_name}>{user.full_name || '(Nome não informado)'}</p>
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    {user.role === 'admin' ? (
                        <span className="text-xs font-bold text-yellow-500 bg-yellow-900/30 px-3 py-1.5 rounded-full w-full text-center sm:w-36">
                            Admin
                        </span>
                    ) : (
                      <select 
                        ...
                        className="w-full sm:w-48 text-xs font-bold py-1.5 px-3 rounded-lg border-gray-600 focus:ring-yellow-500 focus:border-yellow-500 bg-gray-700 text-gray-200"
                      >
